...
function RpcClient(opts) {
    ...
    this.port = opts.port || 6269;
    this.user = opts.user || 'user'; // Should match galactrum.conf
    this.pass = opts.pass || 'pass'; // Should match galactrum.conf
...
function innerRpc(request, callback) {
    ...
    var errorMessage = 'Galactrum JSON-RPC: ';
    ...
    var req = this.protocol.request(options, function (res) {
        ...
        res.on('end', function () {
            ...
            if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
                var exceededError = new Error('Galactrum JSON-RPC: ' + buf.toString('utf8'));
...
// For definitions of RPC calls, see various files in: https://github.com/galactrum/galactrum/tree/master/src
RpcClient.callspec = {
